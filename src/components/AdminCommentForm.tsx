...
export function AdminCommentForm({ productId, onCommentAdded }: AdminCommentFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-white p-4 rounded-lg border border-black/10 mt-4">
      ...
      <button
        type="submit"
        className="inline-flex w-full items-center justify-center rounded-md bg-black px-4 py-2.5 text-white hover:opacity-90 active:opacity-80 disabled:cursor-not-allowed disabled:opacity-60 text-sm sm:w-auto sm:py-2"
        disabled={loading}
      >
